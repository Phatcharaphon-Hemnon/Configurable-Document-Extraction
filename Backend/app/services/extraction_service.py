...
from app.services.field_matching import _normalize_name, build_alternative_name_lookup, values_match
...
class DocumentExtractionService:
    ...
    def evaluate(
        ...
        mismatches = [
            ...
            if not values_match(prediction.get(field_name), expected_value)
        ]

        for field_name, expected_value in ground_truth.items():
            if values_match(prediction.get(field_name), expected_value):
                matched_fields += 1
            else:
                false_negatives += 1

        for field_name, predicted_value in prediction.items():
            if not values_match(predicted_value, ground_truth.get(field_name)):
                false_positives += 1
