...
const { getPipelineStage, getFailedStageLabel, getResultKind, getDisplayDocType } = await load(['src/utils/pipeline.ts']);

const ocrFail = { failed_stage: 'ocr', error: 'OCR text incoherent: the page could not be transcribed reliably', fields: [], validation_errors: [], judge: null, doc_type: 'invoice' };
const routerFail = { failed_stage: 'router', error: 'Router failed: timed out', fields: [], validation_errors: [], judge: null, doc_type: 'invoice' };
const extractorFail = { failed_stage: 'extractor', error: 'Extractor failed: LLM output failed (table_root)', fields: [], validation_errors: [], judge: null, doc_type: 'invoice' };
...
test('OCR/Router failures display Unclassified, extractor keeps routed type', () => {
  assert.equal(getDisplayDocType(ocrFail), 'Unclassified');
  assert.equal(getDisplayDocType(routerFail), 'Unclassified');
  assert.equal(getDisplayDocType(null), 'Unclassified');
  assert.equal(getDisplayDocType(extractorFail), 'invoice');
  assert.equal(getDisplayDocType({ failed_stage: null, doc_type: 'purchase_order' }), 'purchase order');
});
