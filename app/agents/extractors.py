...
class BaseExtractor:
    ...
    def _gemini_extract(
        ...
        prompt = (
            f"You are a document data extraction expert. Extract ALL available fields from the document text below.\n\n"
            f"Document type: {doc_type.value}\n\n"
            f"Fields to extract (extract every field you can find, skip only if truly absent):\n{fields_json}\n\n"
            f"Rules:\n"
            f"- Return a JSON object where each key is a field name from the list above.\n"
            f"- Each value must be an object with: value (the extracted value), confidence (0.0-1.0), source_span (exact text from document).\n"
            f"- For numeric amounts, return the number without currency symbols.\n"
            f"- For dates, preserve the original format from the document.\n"
            f"- For line_items or items, return an array of objects.\n"
            f"- If a field is not present in the document, omit it entirely.\n"
            f"- Do NOT invent or hallucinate values. Only extract what is explicitly in the text.\n\n"
            f"Document text:\n{context.text}\n\n"
...
class InvoiceExtractor(BaseExtractor):
    def extract(self, context: ExtractionContext) -> dict[str, ExtractionField]:
        ...
        if is_bad and "total_amount" in result:
            val = result["total_amount"].value
            if isinstance(val, (int, float)) and val > 0:
                result["total_amount"] = ExtractionField(value=-abs(float(val)), confidence=0.28, source_span=result["total_amount"].source_span)
